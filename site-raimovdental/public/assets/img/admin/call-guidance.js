...
const scripts={
  ...
  homeCare:{id:'HC-01',title:'Закрытие визита · уход и памятка',say:'После процедуры у нас есть короткая памятка — что делать сегодня вечером. Вот она. Если по уходу удобно взять средства здесь — покажу то, что обычно рекомендуют после этой процедуры; если нет — ориентиры в памятке.',next:'Выбрать процедуру в матрице → проговорить скрипт → предложить корзину один раз → распечатать памятку → записать следующий визит.',record:'procedure, memo_id, memo_printed, care_taken|declined, next_visit, doctor_care.',avoid:'Не давить после отказа; не продавать бренды вместо пользы; не пропускать печать памятки.'}
};
let scheduled=false;

...
const scripts = {
  admin_memo_handout:
    'После процедуры у нас есть короткая памятка — что делать сегодня вечером. Вот она. Если по уходу удобно взять средства здесь — покажу то, что обычно рекомендуют; если нет — ориентиры в памятке.',
  doctor_no_brand:

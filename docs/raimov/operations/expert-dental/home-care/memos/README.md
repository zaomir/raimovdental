...
## Обязательный блок «Рецепт для памяти»

В каждой памятке `by-procedure/` — раздел **«Рецепт для памяти — что купить / взять»** (как памятка к рецепту врача, не юридический рецепт):

| Подблок | Содержание |
|---|---|
| Средства ухода | Галочки из `HOME_CARE_MATRIX.json` → `care_basket` (классы без брендов; ☑ = обычно после процедуры) |
| Препараты (аптека) | Пустые строки + подсказки по направлению; **только если назначил врач** — название/дозу вписывают от руки |
| Следующий визит | Из матрицы (`next_visit` / `next_visit_when`) + поле даты записи |
| Особые указания | Пустые строки для рукописных пометок врача/админа |

Пациент уносит лист домой как список покупок и следующего шага. Цен и брендов в памятке нет — артикул показывает администратор у витрины.

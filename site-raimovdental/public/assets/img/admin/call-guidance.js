...
const scripts={
  ...
  homeCare:{id:'HC-01',title:'Закрытие визита · уход и памятка',say:'После процедуры у нас есть короткая памятка — что делать сегодня вечером. Внизу список «что купить / взять», как памятка к рецепту врача. Если удобно взять средства здесь — покажу; если нет — ориентиры уже в памятке.',next:'Выбрать процедуру в матрице → проговорить скрипт → предложить корзину один раз → распечатать памятку (с перечнем к покупке) → записать следующий визит.',record:'procedure, memo_id, memo_printed, care_taken|declined, next_visit, doctor_care.',avoid:'Не давить после отказа; не продавать бренды вместо пользы; не пропускать печать памятки; препараты — только по схеме врача.'}
};
let scheduled=false;

...
2. Памятку пациента **печатает и выдаёт администратор** после процедуры; шаблон **1:1** с позицией прайса; в памятке обязателен блок **«Рецепт для памяти»** (перечень к покупке + следующий визит).  
3. Матрица **процедура → доп. процедура → продажа** обязательна для скриптов и UI врача/админа.  
4. **Полезный подарок** (часто мягкая щётка с тихим брендом) — отдельный контур от розницы: compliance + ежедневное напоминание о клинике; gift ≠ sale (ED-MAT-063).  
5. Врач: **0% с SKU**. Мотивация администратора — после пилота; % не за выдачу подарков.  
6. Розница ухода **не входит** в success fee, пока не согласовано отдельно.  
7. Операционный пакет: ED-MAT-054…063; SSOT: `EXPERT_DENTAL_HOME_CARE_HANDOFF_SYSTEM.md`.
...
3. Gift-SKU мягкой щётки + пилот G0 после гигиены (ED-MAT-063).  
4. Пилот 2 недели (гигиена → терапия → эндо).  
5. Решение по % администратора (не за подарки).

...
export const chief = {
  ...
  /**
   * DEC-801 — developer of RAIM SMILE SYSTEM. Patient benefit only; no outcome guarantee.
   */
  raimSmileSystem: {
    id: 'raim-smile-system',
    kicker: 'RAIM SMILE SYSTEM',
    title: 'Разработчик системы, по которой работает клиника',
    lead:
      'Атабек Раимов разработал RAIM SMILE SYSTEM — именной маршрут сложного эстетического и '
      + 'функционального случая. Expert Dental Studio ведёт приём по этой системе.',
    benefits: [
      {
        title: 'Прикус до эстетики',
        text:
          'Перед винирами или протезированием оценивают окклюзию и сустав — чтобы новая улыбка '
          + 'не ломалась о скрытую функциональную проблему.',
      },
      {
        title: 'Увидеть форму заранее',
        text:
          'Фотопротокол, сканирование, wax-up и мокап во рту дают согласовать вид до необратимой '
          + 'обточки.',
      },
      {
        title: 'Письменный план с ценами',
        text:
          'Последовательность этапов и стоимость фиксируются до начала лечения — без давления '
          + '«решить сегодня».',
      },
      {
        title: 'Наблюдение после лечения',
        text:
          'Контрольные визиты остаются в том же клиническом контуре: система не заканчивается '
          + 'в день фиксации.',
      },
    ],
    note:
      'Система описывает порядок работы. Конкретный результат лечения не обещается до осмотра '
      + 'и диагностики.',
    externalHref: 'https://raimsmile.com/',
    externalLabel: 'raimsmile.com — пациентская витрина системы',
  },
};
